...
> **Windows:** venv để console script ở `.venv/Scripts/` chứ không phải
> `.venv/bin/`; Makefile tự dò nên `make test` / `make notebooks` chạy được ở
> cả ba OS. Nếu máy không có `make`, dùng `python run_notebooks.py` —
> tương đương `make notebooks` (nó tự thêm `.venv/Scripts` vào `PATH`, thứ mà
> NB3 cần để gọi `uvicorn` và NB4 cần để gọi `feast`).
...
make screenshots     Both: render notebook output -> submission/screenshots/*.png
...
├── run_notebooks.py                # execute all notebooks headless (Windows-friendly)
├── notebooks/                      # 8 Jupytext .py files (source of truth) + .ipynb đã chạy
│   ├── _setup.py
│   ├── 01_embeddings_index.py      # core
...
│   ├── embeddings.py               # pluggable backend (EMBEDDING_BACKEND)
│   ├── metadata.py                 # payload schema + selectivity (NB5)
│   ├── filters.py                  # post / pre / filtered-ANN (NB5)
│   ├── agent.py                    # planner + reflection retrieval loop (NB6)
│   ├── cache.py                    # semantic cache, threshold/TTL/namespace (NB7)
│   ├── features.py                 # 6 feature families + leakage probes (NB8)
│   ├── feast_repo/
│   │   ├── feature_store.yaml      # Feast config (lite vs docker)
│   │   └── feature_views.py        # 3 feature views definition
│   └── feast_repo_ondemand/        # on-demand feature view repo (NB8)
├── scripts/
...
│   ├── gen_spend.py                # NB8 user_spend.parquet
│   ├── make_screenshots.py         # render notebook output -> submission/screenshots
│   ├── verify_lite.py              # lite smoke test
│   └── verify_docker.py            # docker smoke test
├── tests/                          # 41 pytest cases (`make test`)
├── bonus/                          # bonus challenge (ARCHITECTURE.md + agent.py + demo.py)
├── data/                           # corpus + golden_set (regenerated by `make seed`)
└── submission/
    ├── REFLECTION.md               # ≤ 200 words
    └── screenshots/                # 1+ ảnh / notebook (`make screenshots`)
```
...
| NB3 P99 > 50ms | Kiểm tra `pip show onnxruntime` trước tiên: **1.29.0 chậm gấp ~10 lần** cho model này (45,9 ms vs 4,0 ms mỗi query). `pip install 'onnxruntime<1.29'`. Nếu vẫn cao thì mới nghĩ tới cold start — chạy 10 query warmup rồi đo lại. |
| NB3 mỗi request mất ~2 giây | Bạn đang gọi `localhost`. Trên Windows nó phân giải `::1` trước còn uvicorn chỉ bind IPv4 → mỗi request chờ IPv6 timeout. Dùng `127.0.0.1`, và dùng chung một `httpx.Client` để có keep-alive. |
